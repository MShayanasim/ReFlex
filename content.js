...
function triggerTranscriptPrint(sems) {
    let printContainer = document.getElementById('ff-print-container');
    if (printContainer) printContainer.remove();
    
    printContainer = document.createElement('div');
    printContainer.id = 'ff-print-container';
    ...
    let html = `
        <div class="ff-print-header">
            <h1>Unofficial Transcript Report</h1>
    ...
    sems.forEach(sem => {
        if (!sem) return; // Safeguard for empty semesters
        
        html += `
            <div class="ff-print-semester">
                <div class="ff-print-sem-header">
                    <h2>${esc(sem.name)}</h2>
                    <div class="ff-print-sem-stats">
                        <span><strong>SGPA:</strong> ${sem.sgpa}</span>
                        <span><strong>CGPA:</strong> ${sem.cgpa}</span>
                    </div>
                </div>
                <table class="ff-print-table">
                    <thead>
                        <tr>
        ...
        sem.courses.forEach(c => {
            ...
            html += `
                        <tr>
                            ...
                            <td>${esc(c.name)}</td>
                            <td>${cr}</td>
                            <td>${(cr * pts).toFixed(2)}</td>
                            <td><strong>${esc(c.grade || '—')}</strong></td>
                            <td>${gpaText}</td>
                        </tr>
            `;
        });
    ...
    html += `
        <div class="ff-print-footer" style="margin-top: 40px; padding-top: 16px; border-top: 1px dashed #cbd5e1; font-size: 0.9rem; color: #64748b; text-align: center;">
    ...
    const isDark = document.documentElement.classList.contains('ff-dark');
    if (isDark) document.documentElement.classList.remove('ff-dark');
    document.body.classList.add('ff-printing');
    
    setTimeout(() => {
        window.print();
        document.body.classList.remove('ff-printing');
        if (isDark) document.documentElement.classList.add('ff-dark');
